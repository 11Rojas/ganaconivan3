...
type PaymentMethod = "venezolano-credito"
...
const paymentMethods: PaymentMethodConfig[] = [
  {
    ...
    icon: CreditCard,
    description: "Pago en Bolívares via Pago Móvil",
    currency: "VES",
    fields: [
...
export default function RaffleCard({ raffle }: { raffle: Raffle }) {
  ...
  const [selectedPayment, setSelectedPayment] = useState<PaymentMethod>('venezolano-credito')
...
<div className="space-y-2">
  ...
  <div className="flex items-center gap-2">
    <Button
      type="button"
      variant="outline"
      onClick={() => setQuantity(prev => Math.max(minTickets, prev - 1))}
    ...
    <Input
      type="number"
      min={minTickets.toString()}
      max="500"
      value={quantity}
      onChange={(e) => {
        const value = parseInt(e.target.value) || minTickets
        setQuantity(Math.max(minTickets, Math.min(500, value)))
      }}
  ...
  <p className="text-sm text-gray-500 text-center">
    {minTickets > 1 ? `Mínimo ${minTickets} tickets, ` : ''}Máximo 500 tickets por compra
  </p>
</div>
